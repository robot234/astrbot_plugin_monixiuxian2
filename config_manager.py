...
class ConfigManager:
    ...
    def __init__(self, base_dir: Path):
        ...
        self.techniques_data: Dict[str, dict] = {}
        ...
        self.adventure_events_config: Dict[str, Any] = {}
    ...
    def _load_all(self):
        """加载所有配置文件"""
        self.level_data = self._load_json_data(self.config_dir / "level_config.json")
        self.body_level_data = self._load_json_data(self.config_dir / "body_level_config.json")
        self.items_data = self._load_items_data(self.config_dir / "items.json")
        self.weapons_data = self._load_items_data(self.config_dir / "weapons.json")
        self.pills_data = self._load_items_data(self.config_dir / "pills.json")
        self.exp_pills_data = self._load_items_data(self.config_dir / "exp_pills.json")
        self.utility_pills_data = self._load_items_data(self.config_dir / "utility_pills.json")
        self.storage_rings_data = self._load_items_data(self.config_dir / "storage_rings.json")
        
        self.skills_data = self._load_json("skills.json")
        self.techniques_data = self._load_json("techniques.json")
        
        self.sect_config = self._load_config_with_default(self.config_dir / "sect_config.json", SECT_CONFIG)
        self.boss_config = self._load_config_with_default(self.config_dir / "boss_config.json", BOSS_CONFIG)
        self.rift_config = self._load_config_with_default(self.config_dir / "rift_config.json", RIFT_CONFIG)
        self.alchemy_config = self._load_config_with_default(self.config_dir / "alchemy_config.json", ALCHEMY_CONFIG)
        self.alchemy_recipes = self._load_items_data(self.config_dir / "alchemy_recipes.json")
        
        self.game_config = self._load_config_with_default(self.config_dir / "game_config.json", {})
        
        self.adventure_events_config = self._load_config_with_default(
            self.config_dir / "adventure_events.json",
            {"events": {}, "trigger_settings": {}, "rarity_weights": {}}
        )
        ...
        logger.info(
            ...
            f"{len(self.adventure_events_config.get('events', {}))} 个奇遇事件，"
    ...
    def get_adventure_events_config(self) -> Dict[str, Any]:
        """获取奇遇事件配置
        
        Returns:
            奇遇事件配置字典
        """
        return self.adventure_events_config
