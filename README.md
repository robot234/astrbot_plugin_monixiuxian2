...
| `闭关` | 开始闭关修炼，离线也可积累修为 |
| `出关` | 结算闭关时间，获取对应修为 |
| `突破信息` | 查看当前境界突破所需修为、心魔难度、减难与风险 |
| `突破 [丹药名]` | 挑战心魔尝试突破，可服用丹药削弱心魔难度 |

### 🎒 装备丹药
| 指令 | 说明 |
|------|------|
| `我的装备` | 显示已装备的武器/防具/饰品/心法 |
| `装备 <名>` | 从储物戒装备到身上 |
| `卸下 <名>` | 卸下装备放回储物戒 |
| `丹药背包` | 查看拥有的丹药及效果 |
| `服用丹药 <名>` | 使用丹药获得增益 |
...
| `取出 <物品名> [数量]` | 从储物戒取出物品用于清除垃圾  |
| `搜索物品 <关键词>` | 模糊搜索储物戒物品 |
| `取出所有 <分类>` | 批量取出指定分类（材料/装备/功法/其他）用于清除垃圾 |
...
| 类别 | 示例 | 作用 | 获取方式 |
|------|------|------|------|
| 破境丹 | 筑基丹、凝婴丹、化神丹 | 直接削弱指定境界突破时的心魔难度，越契合境界效果越明显 | 丹阁刷新、秘境稀有掉落 |
...
| 百宝丹药（即时消耗） | 一品气血丹、三品凝神增益丹 | 购买后立即恢复气血/灵气、增减属性或短暂降低突破时的心魔难度 | 百宝阁限量出售（无法存入背包） |
...
- 修复系统拍卖定时任务错误访问 `ConfigManager.armors_data` 导致的异常
- 系统拍卖改为从 `items.json` 中正确读取防具、饰品配置参与上架
- README、帮助信息与 metadata 版本同步更新

### v3.1.4.10 - 心魔难度二次平衡

- 继续下调前中期心魔基础难度，避免低境界阶段惩罚感过强
- 放缓高资质与低资质的额外偏移幅度，保留差异但减少断层体验
- 略微提高破境丹与临时增益的减难效果，让准备行为更有价值
- 下调后期心魔属性与技能成长上限，减少极端暴击、闪避和技能爆发
- README、帮助信息与 metadata 版本同步更新

### v3.1.4.9 - 心魔难度阶梯重构

- 修复突破系统中心魔资质映射失效的问题，避免多数灵根/体质都按默认难度计算
- 重做心魔难度曲线，改为随境界阶梯化递增，并保留资质偏移与丹药减难
- 心魔属性改为参考玩家当前战斗属性生成，前中后期难度提升更平滑
- 突破信息页、帮助说明与 README 同步更新为“心魔难度/减难”表述
- metadata、README 与 `/修仙帮助` 版本同步更新

### v3.1.4.8 - 灵田完整参数提取修正
- 修复灵田命令仍优先使用框架首个参数、导致 `/种植 九叶灵芝 5` 依旧只种 1 株的问题
- `种植` 与 `批量种植` 现改为优先读取原始消息中的完整参数，再回退到框架参数
- README、帮助信息与 metadata 版本同步更新

### v3.1.4.7 - 灵田多参数命令修复
- 修复 `种植 九叶灵芝 18` 这类多参数写法被框架截断、实际只种 1 株的问题
- 灵田入口现在优先从原始消息中提取完整参数，`种植` 与 `批量种植` 都可稳定识别数量
- README、帮助信息与 metadata 版本同步更新

### v3.1.4.6 - 灵田批量种植增强
- 强化 `种植` 指令参数解析，支持 `灵草 5`、`5 灵草`、`灵草x5`、`灵草 全部` 等批量写法
- 新增 `批量种植 <灵草名> <数量>` 别名命令，降低灵田播种操作成本
- 灵田播种成功提示会显示实际种植数量，并在空位不足时明确提示截断结果
- README、帮助信息与 metadata 版本同步更新

### v3.1.4.5 - 装备/功法命令兼容补强
- 新增 `装备功法`、`装备心法`、`卸下功法`、`卸下心法` 指令别名，降低功法装备失败率
- 重写装备指令参数规范化逻辑，兼容全角空格、前缀误带入和按名称卸下辅修功法
- 修复 `metadata.yaml` 结构异常，确保插件版本信息可被正确读取
- README、帮助信息与 metadata 版本同步更新

### v3.1.4.4 - 装备与功法解析修复
- 修复 `装备 <物品名>` / `装备 <功法名>` 在部分配置下按名称无法匹配的问题
- 装备解析器现在会同时按配置 key 与 `name` 字段回退查找，解决“储物戒里有但提示未找到物品”的情况
- 功法、装备、饰品的名称解析链统一，减少因配置格式差异导致的装备失败
- README、帮助信息与 metadata 版本同步更新

### v3.1.4.3 - 装备位与器阁修复
- 新增正式饰品装备位，饰品现在可以像武器、防具一样正常装备、卸下并写入数据库
- 修复 `装备 <物品名>` 对旧配置解析不完整的问题，统一复用装备解析器，避免“背包里有但无法装备”
- `我的信息` 与 `我的装备` 现已显示饰品栏位，方便查看当前穿戴情况
- 重做器阁装备池抽样，降低武器刷出过多的问题，器阁会更稳定地出现防具与饰品
- 储物戒分类已识别饰品，购买或卸下后的饰品会正确归入装备分类
- README、帮助信息与 metadata 版本同步更新

### v3.1.4.2 - 擂台挑战支持按排名指定
- `擂台挑战` / `战力挑战` 新增按排名挑战，支持 `擂台挑战 3`
- 挑战入口提示文案同步更新，降低擂台玩法理解门槛
- README、帮助信息与 metadata 版本同步更新

### v3.1.4.1 - 擂台信息入口补全
- 新增 `我的擂台` 指令，可查看自己的擂台排名、战绩与挑战冷却
- 为擂台系统补充更直观的查看入口：`擂台`、`擂台排行`
- 帮助信息与 metadata 版本同步更新

### v3.1.4.0 - 功能补全与核心流程修复
- 新增 `送灵石 @某人 数量` 指令，支持玩家间直接赠送灵石
- 修复器阁只展示武器的问题，现可正常刷新并购买防具/饰品
- 重做闭关修为公式，按大境界与小阶段成长，提升合体及以上境界的闭关收益
- 修复擂台挑战胜负判定错误，恢复正常换位与冷却结算
- 修复奇遇选择后无后续、部分结局不触发的问题，并为剧情奇遇增加持久化恢复能力
- README、帮助信息与 metadata 版本同步更新
### v2.9.3 - 排行榜系统全面优化
